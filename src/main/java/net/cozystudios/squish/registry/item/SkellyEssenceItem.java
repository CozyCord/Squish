...
public class SkellyEssenceItem extends SquishBaseItem {
    public SkellyEssenceItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        world.playSound(null, user.getX(), user.getY(), user.getZ(),
                SoundEvents.ENTITY_SKELETON_SHOOT, SoundCategory.PLAYERS,
                0.5F, 0.8F + world.random.nextFloat() * 0.4F);

        if (!world.isClient) {
            SkellyEssenceEntity entity = new SkellyEssenceEntity(world, user);
