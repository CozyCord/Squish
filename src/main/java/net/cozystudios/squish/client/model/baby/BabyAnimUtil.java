...
public final class BabyAnimUtil {
    ...
    public static void applyHeadRotation(ModelPart head, float netHeadYaw, float headPitch) {
        float clampedPitch = MathHelper.clamp(headPitch, -30.0F, 30.0F);
        float clampedYaw   = MathHelper.clamp(netHeadYaw, -45.0F, 45.0F);

        head.yaw = clampedYaw * DEG_TO_RAD;
        head.pitch = clampedPitch * DEG_TO_RAD;
    }
    ...
    public static void animateQuadrupedLegs(
        ...
        float amount = limbSwingAmount * swingScale;

        rightFront.pitch = MathHelper.cos(swing) * 1.4F * amount;
        leftFront.pitch  = MathHelper.cos(swing + (float)Math.PI) * 1.4F * amount;

        rightHind.pitch  = MathHelper.cos(swing + (float)Math.PI) * 1.4F * amount;
        leftHind.pitch   = MathHelper.cos(swing) * 1.4F * amount;
    }
    ...
    public static void animateChickenLegs(ModelPart rightLeg, ModelPart leftLeg, float limbSwing, float limbSwingAmount) {
        float swing = limbSwing * 0.6662F;
        rightLeg.pitch = MathHelper.cos(swing) * 1.4F * limbSwingAmount;
        leftLeg.pitch  = MathHelper.cos(swing + (float)Math.PI) * 1.4F * limbSwingAmount;
    }
